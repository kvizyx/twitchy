...
func TestChannelPointsCreateCustomRewards(t *testing.T) {
	...
	fixture := channelFixture(urlValues("broadcaster_id", "123456"), `{"title":"Hydrate","cost":500}`, channelSuccess(customRewardJSON))
	channelContract(t, "create-custom-rewards", fixture, transport, result.Meta, err)
...
func TestChannelPointsDeleteCustomReward(t *testing.T) {
	...
	fixture := channelFixture(urlValues("broadcaster_id", "123456", "id", "reward-1"), "", channelNoContent())
	channelContract(t, "delete-custom-reward", fixture, transport, result.Meta, err)
}
...
func TestChannelPointsGetCustomReward(t *testing.T) {
	...
	fixture := channelFixture(urlValues("broadcaster_id", "123456", "id", "reward-1", "id", "reward-2", "only_manageable_rewards", "true"), "", channelSuccess(customRewardJSON))
	channelContract(t, "get-custom-reward", fixture, transport, result.Meta, err)
}
...
func TestChannelPointsUpdateRedemptionStatus(t *testing.T) {
	...
	fixture := channelFixture(urlValues("id", "redemption-1", "id", "redemption-2", "broadcaster_id", "123456", "reward_id", "reward-1"), `{"status":"FULFILLED"}`, channelSuccess(body))
	channelContract(t, "update-redemption-status", fixture, transport, result.Meta, err)
}
...
func TestChannelPoints_typedProtocolAndAPIErrors(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			transport := testkit.NewRecordingRoundTripper(testkit.RoundTripResponse{StatusCode: test.status, Header: channelRateHeaders(), Body: test.body})
			client := channelClient(t, transport)
			_, err := client.ChannelPoints.GetCustomReward(context.Background(), helix.GetCustomRewardRequest{BroadcasterID: "123456"})
