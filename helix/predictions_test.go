...
func TestPredictionsCreatePrediction_preservesExactWireAndOutcomes(t *testing.T) {
	transport := testkit.NewRecordingRoundTripper(testkit.RoundTripResponse{StatusCode: http.StatusOK, Header: task23RateHeaders(), Body: task23Fixture(t, "prediction.json")})
...
func TestPredictionsEndPrediction_isOneAttemptOnUnauthorized(t *testing.T) {
	transport := testkit.NewRecordingRoundTripper(testkit.RoundTripResponse{StatusCode: http.StatusUnauthorized, Header: task23RateHeaders(), Body: `{"error":"Unauthorized","status":401,"message":"invalid"}`})
	client := task23Client(t, transport)
